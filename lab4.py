...
def drawEyes():
    ...
    gluSphere(quadratic, 0.05 - ((math.sin(time / 4) + 1) / 2 * 0.025), 100, 100)
    glPopMatrix() 
    ...
    quadratic = gluNewQuadric()
    glTranslatef(-0.3, -0.125, -0.9)
    gluSphere(quadratic, 0.05 - ((math.sin((time + 3.14 / 2.0) / 4) + 1) / 2 * 0.025), 100, 100)
...
def draw():
    ...
    glLightfv(GL_LIGHT0, GL_POSITION, (2 * math.cos(time), -2, 0))                 # Источник света вращаем вместе с елкой
